...
<div class="sm:{{ $class_col ?? '' }} col-span-12 mb-4">
    <div class="relative border border-gray-400 rounded-md px-3 py-2 shadow-sm focus-within:ring-1 focus-within:ring-indigo-600 focus-within:border-indigo-600">
        ...
        <input type="number" name="{{$name??''}}" id="{{$name??''}}" class="block w-full border-0 p-0 text-gray-900 focus-visible:outline-none placeholder-gray-500 py-1 text-base focus:ring-0" autocomplete="off" placeholder="{{$placeholder ?? ''}}" @if($disable == true) disabled @endif>
        {{-- <span class="mt-4 mb-2 text-red-500 col-span-6" v-if="dataForm.errors().has('name')">
